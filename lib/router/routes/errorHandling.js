...
const applicationErrorMapper = (err, req, res, next) => {
    console.error(err);
    if ((0, fhir_works_on_aws_interface_1.isMethodNotAllowed)(err)) {
        next(new http_errors_1.default.MethodNotAllowed(err.message));
        return;
    }
    ...
    if ((0, fhir_works_on_aws_interface_1.isInvalidResourceError)(err) || (0, fhir_works_on_aws_interface_1.isBadRequestError)(err)) {
